...
function loadCalendarData() {
    ...
    fetch(`index.php?route=api&action=calendar&month=${month}&year=${year}&course_id=${selectedCourseId}`)
        .then(res => res.json())
        .then(data => {
...
function renderInterface() {
    ...
    if (list) {
        ...
        if (dailyData.length > 0) {
            ...
            if (isSuperAdmin) {
                // Modo Super Admin: Agrupa por CURSO conforme a imagem
                const grouped = {};
                dailyData.forEach(item => {
                    const courseName = item.curso_nome || 'Curso Indefinido';
                    if (!grouped[courseName]) grouped[courseName] = { count: 0 };
                    grouped[courseName].count++;
                });

                Object.keys(grouped).forEach(courseName => {
                    const data = grouped[courseName];
                    list.innerHTML += `
                        <div class="occurrence-item" onclick="applyCourseFilterByName('${courseName.replace(/'/g, "\\'")}')" style="cursor:pointer; display: flex; align-items: center; gap: 12px; padding: 12px; border-bottom: 1px solid #f8fafc;">
                            <div class="occ-avatar" style="background: #f1f5f9; color: #64748b; border-radius: 8px; width: 36px; height: 36px; display: flex; align-items: center; justify-content: center; font-size: 12px; font-weight: 700;">
                                <i data-lucide="layers" style="width: 18px; height: 18px;"></i>
                            </div>
                            <div class="occ-info" style="flex: 1;">
                                <span class="occ-name" style="font-weight:600; font-size: 13px; color: #1e293b; display: block;">${courseName}</span>
                                <span class="occ-desc" style="color:#ef4444; font-weight:600; font-size: 11px;">${data.count} ocorrência${data.count > 1 ? 's' : ''} encontrada${data.count > 1 ? 's' : ''}</span>
                            </div>
                            <div class="occ-status-icon" style="color: #fee2e2;">
                                <i data-lucide="alert-circle" style="width: 16px; height: 16px; fill: #ef4444; color: white;"></i>
                            </div>
                        </div>`;
                });
...
function exportData() {
    ...
    fetch(`index.php?route=api&action=modal_details&month=${currentMonth}&year=${currentYear}`)
        .then(res => res.json())
        .then(data => {
...
function openDetailModal(monthIndex, monthName, epiName = '') {
    ...
    let url = `index.php?route=api&action=modal_details&month=${realMonth}&year=${currentYear}&course_id=${selectedCourseId}`;
    if (epiName) {
...
function loadCharts() {
    fetch(`index.php?route=api&action=charts&course_id=${selectedCourseId}`)
        .then(res => res.json())
        .then(response => {
